...
if((isset($_SESSION)) && (isset($_SESSION['uid']))) 
{
	$id=$_SESSION['uid'];
		...
		if(isset($_GET['ac']))
		{
			...
			if($_GET['ac'] =="chy")
			{
				$qry="UPDATE `tbl_courier_officers` SET `off_pwd`='".$_POST['gstincon']."' WHERE `office`='".$id."' and `status`='A'";
				$sql=mysqli_query($dbConn,$qry);
			
			if($sql)
				{
					echo "<script>alert('Updated. Please Login Again with New Password!');window.location.href = 'logout.php';</script>";
				}
			else
				{
...
<?php
	...
	$sqlms=mysqli_query($dbConn,"Select * from tbl_courier_officers where office = '".$id."'");
	...
	?>		<div class="page-content">
						...
						<div class="row">
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
								<form class="form-horizontal" method="POST" action="changepwd.php?ac=chy">
									
									<div class="form-group">
										<label class="control-label col-xs-12 col-sm-3 no-padding-right" for="consignor">Branch Name:</label>
											<div class="col-xs-12 col-sm-9">
												<div class="clearfix">
													<input type="text" name="consignor" id="consignor" class="col-xs-12 col-sm-6" value="<?php echo $rowms['officer_name'];?>" required readonly />
												</div>
											</div>
									</div>
